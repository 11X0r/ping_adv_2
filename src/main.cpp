...
int main(int argc, char* argv[]) {
    try {
        ...
        const auto interval = std::chrono::duration<double, std::milli>(std::stod(argv[3]));
        ...
        std::cout << "[" << ip << "] ";
        if (stats.is_valid()) {
            const auto [avg_latency, jitter] = std::tuple{stats.average_latency_ms(), stats.jitter_ms()};
            std::cout << std::fixed << std::setprecision(2)
                     << "Test Result: Average Latency " 
                     << avg_latency 
                     << "ms, Jitter " << jitter
                     << "ms (" << stats.count() << " results)\n";
            return 0;
        } else {
