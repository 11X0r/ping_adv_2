...
int ping_send_probe(struct ping_rts *rts, struct socket_st *sock, void *packet,
  ...
  return sendto(sock->fd, packet, packet_size, 0,
                (struct sockaddr *)&rts->whereto, sizeof(rts->whereto));
}
...
PingWrapper::~PingWrapper() { cleanup(); }
...
PingWrapper::execute(int count,
  ...
  for (int i = 0; i < count && failures < max_failures; ++i) {
    ...
    if (iputils::ping_send_probe(rts_.get(), sock_.get(), rts_->outpack,
                                 sizeof(rts_->outpack)) < 0) {
      failures++;
      continue;
    }
    ...
    if (iputils::ping_parse_reply(rts_.get(), sock_.get(), &msg,
                                  recvmsg(sock_->fd, &msg, 0), &rts_->whereto,
                                  &timeout) == 0) {
